...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  private _leaseTableColumns: TableColumn[] = [];
  ...
  // leaseTableColumns
  get leaseTableColumns(): TableColumn[] {
    return this._leaseTableColumns;
  }
  set leaseTableColumns(value: TableColumn[]) {
  ...
  private async organizeLeaseTableData(): Promise<void> {
    try {
      ...
      this.leaseTableColumns = [
        ...
        {label: 'View', key: 'buttons'},
        {label: 'Download', key: 'buttons'}
      ];
  ...
  //<========================================================================= LEASE BUTTON OPERATIONS =========================================================================>
  protected async handleLeaseOperations(data: TableButtonActionConfig): Promise<void> {
    try {
      if(this.authService.getLoggedUser === null) throw new Error("Please login first!")
      if(!data) throw new Error('Invalid Button Operation');
      const buttonAction = data.action.toLocaleLowerCase().trim();
      const leaseID = data.data.element.leaseid.trim();
        ...
        case 'download':
          try {
            ...
            if(err instanceof HttpErrorResponse) {
              this.NotificationDialogComponent.notification('error', err.message);
            }
            else if(typeof err === 'string') {
              this.NotificationDialogComponent.notification('error', err);
            }
