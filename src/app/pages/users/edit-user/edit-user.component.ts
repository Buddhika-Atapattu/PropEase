...
export class EditUserComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private assignValues(): void {
    try {
      ...
      this.typedCountry = typeof this.user.address.country === 'string' ? this.user.address.country : '';
      console.log( this.typedCountry );
      this.autoSelectCountryOnInit( this.typedCountry );
  ...
  private autoSelectCountryOnInit( stored: string ): void {
    ...
    const found: Country | undefined =
      this.countries.find( ( x ) => x.name.trim().toLowerCase() === q.toLowerCase() ) ??
      this.countries.find( ( x ) => x.code.trim().toLowerCase() === q.toLowerCase() ) ??
      this.countries.find( ( x ) => x.name.toLowerCase().includes( q.toLowerCase() ) );

    if ( !found ) {
      // keep what DB had visible, but no selection
      this.selectedCountry = null;
      this.typedCountry = raw;
      this.filteredCountries = of( this.countries.slice() );
      return;
    }

    // ✅ Minimal event object (only what your handler uses)
    const fakeEvent = {
      option: { value: found },
    } as unknown as MatAutocompleteSelectedEvent;


    // optional: stabilize dropdown list
    this.selectedCountry = found;
    console.log( this.selectedCountry );

    // ✅ reuse your existing handler
    this.onCountrySelected( fakeEvent );
  }
 ...
 */
  ...
  protected async updateUser(): Promise<void> {
    ...
    try {
      ...
      const country = this.selectedCountry?.name ?? this.typedCountry ?? '';
      if ( !country )
