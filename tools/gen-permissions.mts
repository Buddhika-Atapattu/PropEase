...
class PermissionsGenerator {
  ...
  static ACCESS_MAP_PATH = path.join(
    PermissionsGenerator.PROJECT_ROOT,
    "src",
    "app",
    "source",
    "access-map.source.ts"
  );
  static OUT_PATH = path.join(
    PermissionsGenerator.PROJECT_ROOT,
    "src",
    "app",
    "core",
  ...
  public static async run(): Promise<void> {
    try {
      ...
      if (!accessOptions.length) {
        throw new Error(
          "ACCESS_OPTIONS parsed as empty. Check access-map.source.ts structure."
        );
      }
      ...
      console.log(
        `[Success:] [PermGen] Generated: ${this.#relativeToRoot(
          this.OUT_PATH
        )}\n`
      );
      console.log(
        `[Info:] [PermGen] Source: ${this.#relativeToRoot(
          this.ACCESS_MAP_PATH
        )}\n`
      );
    } catch (error) {
  ...
  static #relativeToRoot(absPath: string): string {
    return path.relative(this.PROJECT_ROOT, absPath).replaceAll("\\", "/");
  }

  static #resolveProjectRoot(): string {
    // Prefer where the command is executed from (npm run gen:perm runs at repo root)
    // But make it robust by walking up until we find package.json.
    ...
    // Fallback: still better than resolving from tools-dist
    return process.cwd();
  }
   ...
   */
  static #extractAccessOptionsFromTsAst(sourceText: string): Array<{
    module: string;
    actions: string[];
  }> {
    ...
    let accessOptionsNode: ts.Node | null = null;
    ...
    if (!accessOptionsNode) {
      throw new Error(
        "Could not find ACCESS_OPTIONS initializer in access-map.source.ts"
      );
    }
    ...
    if (!arrayExpr) {
      throw new Error(
        "ACCESS_OPTIONS initializer is not an array literal after unwrapping."
      );
    }

    const results: Array<{ module: string; actions: string[] }> = [];
    ...
    for (const el of arrayExpr.elements) {
      ...
      if (actionsArray) {
        for (const actionEl of actionsArray.elements) {
          if (!ts.isObjectLiteralExpression(actionEl)) continue;
  ...
  static #unwrapToArrayLiteral(node: ts.Node): ts.ArrayLiteralExpression | null {
   ...
   */
  static #buildPermTree(
    accessOptions: Array<{ module: string; actions: string[] }>
  ): Record<string, Record<string, { module: string; action: string }>> {
    const tree: Record<string, Record<string, { module: string; action: string }>> = {};
  ...
  // Render output TS
  // ──────────────────────────────────────────────────────────────────────────
  static #renderOutputTs(
    permTree: Record<string, Record<string, { module: string; action: string }>>
  ): string {
